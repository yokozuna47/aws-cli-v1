...
|------------------------|----------------------------|---------------------------------|
| VPC par défaut         | `À_REMPLIR`                | 172.31.0.0/16 (non modifié)     |
| Sous-réseau dédié      | `À_REMPLIR`                | À_REMPLIR, us-east-1a           |
| Paire de clés          | `cle-td-jilani`            | fichier `cle-td-jilani.pem`     |
| Instance **bastion**   | `À_REMPLIR`                | priv À_REMPLIR / pub À_REMPLIR  |
| Instance **cible**     | `À_REMPLIR`                | priv À_REMPLIR / pas d'IP publique |
| SG bastion             | `À_REMPLIR`                | `td-bastion-jilani`             |
| SG cible               | `À_REMPLIR`                | `td-cible-jilani`               |
| NACL personnalisée     | `À_REMPLIR`                | `td-nacl-jilani`                |
| NACL par défaut        | `À_REMPLIR`                | réassociée à la fin (non supprimée) |
...
### 1. Prérequis
```bash
curl https://checkip.amazonaws.com            # -> MON IP = À_REMPLIR/32
...
```bash
# sous-réseau dédié
aws ec2 create-subnet --vpc-id À_REMPLIR \
...
aws ec2 run-instances --image-id À_REMPLIR --instance-type t3.micro \
  --key-name cle-td-jilani --subnet-id À_REMPLIR \
...
aws ec2 authorize-security-group-ingress --group-id À_REMPLIR \
  --protocol tcp --port 22 --cidr À_REMPLIR/32

# sg-cible : SSH + ICMP dont la SOURCE est sg-bastion (pas une plage d'IP)
aws ec2 authorize-security-group-ingress --group-id À_REMPLIR \
  --protocol tcp --port 22 --source-group À_REMPLIR
aws ec2 authorize-security-group-ingress --group-id À_REMPLIR \
  --protocol icmp --port -1 --source-group À_REMPLIR
```
Mon test (j'ai utilisé l'agent forwarding pour rebondir vers la cible) :
```bash
ssh-add cle-td-jilani.pem
ssh -A -i cle-td-jilani.pem ec2-user@À_REMPLIR   # bastion
# depuis le bastion :
ping -c 3 À_REMPLIR            # OK (ICMP via sg-bastion)
ssh ec2-user@À_REMPLIR         # OK (SSH via sg-bastion) -> réponse auto (stateful)
```

### 5. NACL (stateless)
```bash
# j'autorise l'entrée SSH...
aws ec2 create-network-acl-entry --network-acl-id À_REMPLIR \
  --rule-number 100 --protocol 6 --port-range From=22,To=22 \
  --cidr-block À_REMPLIR/32 --rule-action allow --ingress
...
aws ec2 create-network-acl-entry --network-acl-id À_REMPLIR \
  ...
  --cidr-block À_REMPLIR/32 --rule-action deny --ingress
# je la supprime -> accès rétabli
aws ec2 delete-network-acl-entry --network-acl-id À_REMPLIR \
  --rule-number 90 --ingress
```

### 7. Nettoyage
```bash
aws ec2 terminate-instances --instance-ids À_REMPLIR À_REMPLIR
aws ec2 wait instance-terminated --instance-ids À_REMPLIR À_REMPLIR
# je rétablis la NACL par défaut puis je supprime ma NACL custom
aws ec2 replace-network-acl-association --association-id À_REMPLIR \
  --network-acl-id À_REMPLIR
aws ec2 delete-network-acl --network-acl-id À_REMPLIR
# SG (cible d'abord car elle référence le bastion), puis subnet et clés
aws ec2 delete-security-group --group-id À_REMPLIR
aws ec2 delete-security-group --group-id À_REMPLIR
aws ec2 delete-subnet --subnet-id À_REMPLIR
...
1. **Agent forwarding (`ssh -A`).** Pour rebondir vers la cible, j'ai utilisé `ssh -A`, ce
   ...
   ```bash
   ssh -J ec2-user@À_REMPLIR ec2-user@À_REMPLIR
   ```
...
3. **Aucune couche de détection.** Je n'ai mis que du préventif (SG/NACL), pas de détectif :
   ni **VPC Flow Logs**, ni surveillance CloudTrail.
4. **SSH public.** Même limité à mon IP, le bastion garde un port 22 ouvert sur Internet.
5. **Approche « encore mieux » : supprimer le bastion.** La best practice AWS moderne est
   **SSM Session Manager** : zéro port ouvert, zéro clé SSH, accès entièrement audité.

...
**Étudiant :** jilani · Mastère Cybersécurité — BC Design Systems / IPSSI
...
```
                 Internet
                    │  SSH (22) depuis MON IP : À_REMPLIR/32
                    ▼
        ┌───────────────────────────────────────────────┐
        ...
        │   ── NACL td-nacl-jilani (stateless, niveau subnet) ── │
        │                                                 │
        │   ┌────────────────┐        ┌────────────────┐  │
        │   │   BASTION       │  SSH   │    CIBLE       │  │
        │   │ IP publique     │  +ping │ PAS d'IP       │  │
        │   │ À_REMPLIR       │──────► │ publique       │  │
        │   │ priv À_REMPLIR  │        │ priv À_REMPLIR │  │
        │   │ [sg-bastion]    │        │ [sg-cible]     │  │
        │   └────────────────┘        └────────────────┘  │
        └───────────────────────────────────────────────┘

 sg-bastion : entrée SSH(22) depuis À_REMPLIR/32
 sg-cible   : entrée SSH(22) + ICMP dont la SOURCE = sg-bastion (pas une IP)
```

## 2. Règles appliquées

**Security Group `sg-bastion-jilani`** — entrée : TCP 22 depuis `À_REMPLIR/32`.
...
|-----|---------|-----------|--------------|--------------------|--------|
| 90  | entrant | TCP       | 22           | À_REMPLIR/32       | DENY *(test §défense, puis retirée)* |
| 100 | entrant | TCP       | 22           | À_REMPLIR/32       | ALLOW  |

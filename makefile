...
tr_output: ## genere l inventaire ansible depuis les outputs terraform
	...
	@echo "$$(cd $(TF_DIR) && terraform output -raw instance_public_ip) ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/yokozuna_key" >> $(ANSIBLE_DIR)/inventory.ini

wait: ## attend que le SSH de la VM reponde
	@echo "Attente du SSH..."
	@IP=$$(cd $(TF_DIR) && terraform output -raw instance_public_ip); \
	for i in $$(seq 1 30); do \
	ssh -o BatchMode=yes -o StrictHostKeyChecking=no -o ConnectTimeout=5 -i ~/.ssh/yokozuna_key ubuntu@$$IP true 2>/dev/null && { echo " SSH pret ($$IP)"; exit 0; }; \
	printf '.'; sleep 5; \
